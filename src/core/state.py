    ...
    try:
        ...
        try:
            ...
            if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
                raise OSError(f"Unsafe firewall audit file: {path}")
            if path == _AUDIT_LOG_FILE and info.st_uid != 0:
                raise OSError(f"Firewall audit file is not root-owned: {path}")
...
@dataclass
class RollbackGuard:
    """Detached rollback process and its root-controlled armed token."""

    process: subprocess.Popen
    token: Path


def arm_rollback_guard(backup: Path, timeout: int = 65) -> RollbackGuard:
    """Arm a detached rollback that survives termination of the apply process."""
    if timeout <= 0:
        raise ValueError("rollback guard timeout must be positive")
    lock_fd = getattr(_LOCK_LOCAL, "fd", None)
    if lock_fd is None:
        raise RuntimeError("Rollback guard requires the firewall transaction lock")
    ...
    fd, token_name = tempfile.mkstemp(prefix="apply-", suffix=".armed", dir=_LOCK_DIR)
    token = Path(token_name)
    os.fchmod(fd, 0o600)
    os.close(fd)
    script = (
        'delay="$1"; token="$2"; backup="$3"; '
        '/bin/sleep "$delay"; '
        'if [ -e "$token" ]; then /usr/sbin/nft --file "$backup"; fi; '
        '/bin/rm -f -- "$token"'
    )
    try:
        process = subprocess.Popen(
            ["/bin/bash", "-c", script, "rollback-guard", str(timeout), str(token), str(backup)],
    ...
    if process.poll() is not None:
        token.unlink(missing_ok=True)
        raise RuntimeError("Rollback guard exited before ruleset apply")
    return RollbackGuard(process=process, token=token)


def disarm_rollback_guard(guard: RollbackGuard) -> None:
    """Stop a rollback guard before removing its armed token."""
    if guard.process.poll() is None:
        try:
            os.killpg(guard.process.pid, signal.SIGTERM)
...
def _update_persistent_sets(mutator, *, path: Path | None = None) -> dict[str, list[str]]:
    ...
    resolved = path or _SETS_STATE_FILE
    with _persistent_sets_lock(resolved):
        sets = _load_persistent_sets_unlocked(resolved)
        mutator(sets)
        _save_persistent_sets_unlocked(sets, resolved)
        return sets
...
def merge_live_sets_into_persistent() -> dict[str, list[str]]:
    ...
    live_sets: dict[str, list[str]] = {}
    for set_name in _KNOWN_SETS:
        live = set_list(set_name, persistent_fallback=False)
        if live:
            live_sets[set_name] = live

    def mutate(sets):
        for set_name, live in live_sets.items():
            sets[set_name] = sorted(set(sets.get(set_name, [])) | set(live))

    return _update_persistent_sets(mutate)
